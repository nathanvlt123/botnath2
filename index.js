	...
	client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `Já vai tarde babaca @${num.split('@')[0]}👋`
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
		...
		try {
			...
			mess = {
				...
				only: {
					...
					admin: '❌ Este comando só pode ser usado por adm do grupo! ❌',
					Badmin: '❌ Este comando só pode ser usado quando o bot é adm! ❌'
				}
			}

			const botNumber = client.user.jid
			const ownerNumber = ["5541999772028@s.whatsapp.net"] // replace this with your number
			...
			switch(command) {
				...
				case 'info':
					...
					break
				case 'blocklist':
					teks = 'A lista de pessoas bloqueadas pelo bot :\n'
					...
					if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
						...
						await ffmpeg(`./${media}`)
							...
							.on('error', function (err) {
								...
								reply(`❌ Falha ao converter $ {type} na figurinha`)
							})
							.on('end', function () {
								console.log('Finish')
								client.sendMessage(from, fs.readFileSync(ran), sticker, {quoted: mek})
				...
				case 'clearall':
					...
					reply('Todas as mensagens do chat foram removidas com sucesso :)')
					break
				case 'bc':
					if (!isOwner) return reply('Quem é Você?')
					...
					if (mentioned.length > 1) {
						...
						mentions(`Parabens para o novo adm @${mentioned[0].split('@')[0]} respeitem ele!`, mentioned, true)
				...
				case 'demote':
					...
					if (mentioned.length > 1) {
						...
						mentions(`Ih ala perdeu o adm foi?kkk @${mentioned[0].split('@')[0]} Mandem F no chat kkk!`, mentioned, true)
